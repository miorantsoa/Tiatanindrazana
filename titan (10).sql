...
--
-- Client :  localhost
-- Généré le :  Mar 23 Mai 2017 à 17:57
...
(8, 'Andriamatoa', 'Miorantsoa', 'Rakotozafy', '1995-10-22', '201011021355', '2013-12-31', 'Tana', 'upload/infouser/Miorantsoa_Rakotozafy-Miorantsoa-p', 'upload/infouser/Miorantsoa_Rakotozafy-Miorantsoa-p', 'm@gmail.com', 'Miorantsoa', 'momo', 1, '973754', 'upload/infouser/Miorantsoa_Rakotozafy-Miorantsoa-pdp8.jpg'),
(9, 'Andriamatoa', 'Mike', 'Rakotozafy', '1992-10-15', '205045211300', '2017-03-16', 'Antananarivo', 'upload/infouser/Mike-Rakotozafy-pdp16.jpg', 'upload/infouser/Mike-Rakotozafy-pdp17.jpg', 'mikerak@outlook.fr', 'Rakoto', 'kotokely', 0, NULL, 'upload/infouser/Mike-Rakotozafy-pdp15.jpg'),
(12, 'Andriamatoa', 'Rakotozafy', 'Miorantsoa', '2017-06-19', '205045211300', '2013-12-31', 'Fianarantsoa', 'upload/infouser/Rakotozafy-Miorantsoa-pdp25.jpg', 'upload/infouser/Rakotozafy-Miorantsoa-pdp26.jpg', 'mikerak@outlook.fr', 'Mike', 'root', 1, NULL, 'upload/infouser/Rakotozafy-Miorantsoa-pdp24.jpg');
...
INSERT INTO `abonnement` (`idabonnement`, `idtypeabon`, `idutilisateur2`, `datedebutabonnement`, `datefinabonnement`) VALUES
(2, 2, 8, '2017-04-04', '2017-04-18'),
(4, 1, 12, '2017-05-16', '2017-08-14');
...
INSERT INTO `admin` (`idadministrateur`, `nom`, `prenom`, `identifiant`, `motdepasse`, `email`, `statutadministrateur`) VALUES
(1, 'Admin', 'Tia tanindrazana', 'titan_admin', 'administratorX54', 'admin@titan.mg', 0);
...
CREATE TABLE `article` (
  ...
  `url_tag` varchar(200) DEFAULT NULL,
...
(12, 37, 'miorantsoaRak@gmail.com', 'Miorantsoa Rakotozafy', '2017-04-22 09:16:00', 'ok!');
...
--
-- Doublure de structure pour la vue `detail_corruption`
--
CREATE TABLE `detail_corruption` (
`idcorruption` int(11)
,`idcatcorruption` int(11)
,`datedenonciation` date
,`datefait` date
,`nomdenonciateur` varchar(255)
,`adressedenonciateur` varchar(75)
,`telephonedenonciateur` varchar(30)
,`emaildenonciateur` varchar(75)
,`sujet` varchar(255)
,`contenue` text
,`lieu` varchar(100)
,`cheminmedia` varchar(100)
...
INSERT INTO `favoris` (`idutilisateur`, `idarticle`) VALUES
(8, 4),
(8, 36);
...
INSERT INTO `infoutil` (`idbeinfo`, `idcatbeinfo`, `titre`, `url_tag`, `contenue`, `lienphoto`, `dernieremaj`, `copyrightphoto`, `lien`, `publie`) VALUES
(1, 18, 'Lorem Ipsum test', 'pharmacie/Lorem-Ipsum-test--1', 'Lorem ipsum dolor sit amet, consectetur adipisicing elit. Architecto at beatae, blanditiis debitis deleniti dolore excepturi explicabo incidunt, laboriosam mollitia neque non odit officiis qui quisquam, sint suscipit velit voluptas.\r\n', 'upload/info_utile/17-03-24-1490340146.jpg', '2017-03-24 00:00:00', 'uefa', NULL, 1),
(2, 8, 'Procédure judiciaire', 'procédure-administrative/Procdure-judiciaire--2', 'OK juste un test bedeeveveveveve', NULL, '2017-03-24 00:00:00', '', '', 1);
...
CREATE TABLE `journal` (
  ...
  `numeroparution` decimal(8,0) NOT NULL DEFAULT '0',
  `datepublication` date NOT NULL,
  `liencouverture` varchar(500) DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
(6, '875421', '2017-04-03', 'upload/couverture/couverture-17-04-04-1491294535.jpg'),
(7, '4000', '2017-05-12', 'upload/couverture/couverture-17-05-12-1494575283.jpg'),
(8, '54210', '2017-05-23', 'upload/couverture/couverture-17-05-23-1495545925.PNG');
...
(2, '2017-04-22', '2017-05-03', 'Annonce Titan', 'http://tiatanindrazana.com', 'upload/pub/17-04-22-1492843353.jpg', NULL, 1);
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `detail_article`  AS  select `article`.`idarticle` AS `idarticle`,`article`.`idjournal` AS `idjournal`,`article`.`idcategorie` AS `idcategorie`,`article`.`idadministrateur` AS `idadministrateur`,`article`.`dateparution` AS `dateparution`,`article`.`titre` AS `titre`,`article`.`extrait` AS `extrait`,`article`.`resume` AS `resume`,`article`.`contenue` AS `contenue`,`article`.`laune` AS `laune`,`article`.`etatpublication` AS `etatpublication`,`article`.`niveau` AS `niveau`,`article`.`lien_image_une` AS `lien_image_une`,`article`.`url_tag` AS `url_tag`,`article_categories`.`idmere` AS `idmere`,`article_categories`.`catmere` AS `catmere`,`article_categories`.`libelle` AS `libelle`,`article_categories`.`niveau` AS `level`,`journal`.`numeroparution` AS `numeroparution`,`journal`.`datepublication` AS `datepublication`,`journal`.`liencouverture` AS `liencouverture` from ((`article` join `journal` on((`article`.`idjournal` = `journal`.`idjournal`))) join `article_categories` on((`article`.`idcategorie` = `article_categories`.`idcategorie`))) order by `journal`.`datepublication` desc ;

-- --------------------------------------------------------

--
-- Structure de la vue `detail_corruption`
--
DROP TABLE IF EXISTS `detail_corruption`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `detail_corruption`  AS  select `corruption`.`idcorruption` AS `idcorruption`,`corruption`.`idcatcorruption` AS `idcatcorruption`,`corruption`.`datedenonciation` AS `datedenonciation`,`corruption`.`datefait` AS `datefait`,`corruption`.`nomdenonciateur` AS `nomdenonciateur`,`corruption`.`adressedenonciateur` AS `adressedenonciateur`,`corruption`.`telephonedenonciateur` AS `telephonedenonciateur`,`corruption`.`emaildenonciateur` AS `emaildenonciateur`,`corruption`.`sujet` AS `sujet`,`corruption`.`contenue` AS `contenue`,`corruption`.`lieu` AS `lieu`,`media`.`cheminmedia` AS `cheminmedia`,`media`.`alt` AS `alt`,`categoriecorruption`.`libelle` AS `libelle` from (((`corruption` join `fichiercorruption` on((`corruption`.`idcorruption` = `fichiercorruption`.`idcorruption`))) join `media` on((`fichiercorruption`.`idmedia` = `media`.`idmedia`))) join `categoriecorruption` on((`corruption`.`idcatcorruption` = `categoriecorruption`.`idcatcorruption`))) ;
...
--
ALTER TABLE `abonnee`
  MODIFY `idutilisateur2` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=13;
--
-- AUTO_INCREMENT pour la table `abonnement`
--
ALTER TABLE `abonnement`
  MODIFY `idabonnement` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=5;
--
...
--
ALTER TABLE `article`
  MODIFY `idarticle` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=47;
--
...
--
ALTER TABLE `journal`
  MODIFY `idjournal` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=9;
--
...
--
ALTER TABLE `publicite`
  MODIFY `idpublicite` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=3;
--
